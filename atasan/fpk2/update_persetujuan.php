...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result_select_kodeFPK->num_rows > 0) {
        ...
        if ($result_check_employee->num_rows > 0) {
            ...
            $sql_update_persetujuan = "UPDATE persetujuan SET PersetujuanAtasan = 'Disetujui', PersetujuanDireksi2 = 'Disetujui', Status_Penyetujuan = ? WHERE kodeFPK = ?";
            ...
            if ($stmt_update_persetujuan->execute()) {
                ...
                $sql_update_fpk = "UPDATE fpk SET persetujuanAtasan = 'Disetujui', persetujuanDireksi2 = 'Disetujui' WHERE id_fpk = ?";
                $stmt_update_fpk = $connection->prepare($sql_update_fpk);
                $stmt_update_fpk->bind_param("i", $id_fpk);

                if ($stmt_update_fpk->execute()) {
                    // Set tanggal direksi2 jika persetujuan HR Unit disetujui
                    if ($stmt_update_fpk->affected_rows > 0) {
                        $today = date("Y-m-d"); // Ambil tanggal hari ini

                        // Update field tgldireksi2 di tabel fpk
                        $sql_update_tgl_direksi2 = "UPDATE fpk SET tglDireksi2 = ? WHERE id_fpk = ?";
                        $stmt_update_tgl_direksi2 = $connection->prepare($sql_update_tgl_direksi2);
                        $stmt_update_tgl_direksi2->bind_param("si", $today, $id_fpk);
                        
                        if ($stmt_update_tgl_direksi2->execute()) {
                            echo "success";
                        } else {
                            echo "error_update_tgl_direksi2";
                        }

                        // Tutup statement pembaruan tgldireksi2
                        $stmt_update_tgl_direksi2->close();
                    }
                } else {
            ...
            if ($stmt_insert_persetujuan->execute()) {
                ...
                $sql_update_fpk = "UPDATE fpk SET persetujuanDireksi2 = 'Disetujui' WHERE id_fpk = ?";
                $stmt_update_fpk = $connection->prepare($sql_update_fpk);
                $stmt_update_fpk->bind_param("i", $id_fpk);
