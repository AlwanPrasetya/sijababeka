...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result_select_kodeFPK->num_rows > 0) {
        ...
        if ($result_check_employee->num_rows > 0) {
            ...
            $peresetujuan_direksi2 = $row_persetujuan['PersetujuanDireksi2'];
            $persetujuan_id = $row_persetujuan['ID'];

            // Jika sudah ada, lakukan pembaruan nilai PersetujuanDireksi2
            $sql_update_persetujuan = "UPDATE persetujuan SET PersetujuanDireksi2 = 'Ditolak' WHERE kodeFPK = ?";
            ...
            if ($stmt_update_persetujuan->execute()) {
                ...
                $sql_update_fpk = "UPDATE fpk SET PersetujuanDireksi2 = 'Ditolak' WHERE id_fpk = ?";
                ...
                if ($stmt_update_fpk->execute()) {
                    ...
                    if ($peresetujuan_direksi2 == 'Disetujui') {
            ...
            // Tutup statement pembaruan PersetujuanDireksi2
            $stmt_update_persetujuan->close();
        } else {
            // Jika belum ada, tambahkan baris baru dengan kodeFPK tersebut
            $sql_insert_persetujuan = "INSERT INTO persetujuan (kodeFPK, PersetujuanAtasan, PersetujuanDireksi2, PersetujuanAdmin, Status_Penyetujuan) 
