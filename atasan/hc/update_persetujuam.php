...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  $id = $_POST['id'];
  $tglAtasan = date('Y-m-d'); // Mendapatkan tanggal hari ini dalam format YYYY-MM-DD

  // Start transaction
  $connection->begin_transaction();

  try {
    // Update status persetujuan, persetujuanAtasan, dan tglAtasan di tabel hcc
    $sql_hcc = "UPDATE hcc SET Status_Penyetujuan='Approved', persetujuanAtasan='Disetujui', tglAtasan=? WHERE fpk_selection=?";
    ...
    $stmt_hcc->bind_param("ss", $tglAtasan, $id);
    ...
    $sql_persetujuan_hc = "UPDATE persetujuan_hc SET persetujuanAtasan='Disetujui' WHERE fpk_selection=?";
