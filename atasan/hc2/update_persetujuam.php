...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  $id = $_POST['id'];
  $tglDireksi2 = date('Y-m-d'); // Mendapatkan tanggal hari ini dalam format YYYY-MM-DD

  // Start transaction
  $connection->begin_transaction();

  try {
    // Update status persetujuan, persetujuanDireksi2, dan tglDireksi2 di tabel hcc
    $sql_hcc = "UPDATE hcc SET Status_Penyetujuan='Approved', persetujuanDireksi2='Disetujui', tglDireksi2=? WHERE fpk_selection=?";
    ...
    $stmt_hcc->bind_param("ss", $tglDireksi2, $id);
    ...
    $sql_persetujuan_hc = "UPDATE persetujuan_hc SET persetujuanDireksi2='Disetujui' WHERE fpk_selection=?";
