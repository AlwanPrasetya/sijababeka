...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result_select_kodeFPK->num_rows > 0) {
        ...
        $sql_check_employee = "SELECT ID, PersetujuanAdmin, PersetujuanDireksi3 FROM persetujuan WHERE kodeFPK = ?";
        ...
        if ($result_check_employee->num_rows > 0) {
            ...
            $persetujuan_direksi3 = $row_persetujuan['PersetujuanDireksi3'];
            $persetujuan_id = $row_persetujuan['ID'];

            // Jika sudah ada, lakukan pembaruan nilai PersetujuanDireksi3
            $sql_update_persetujuan = "UPDATE persetujuan SET PersetujuanDireksi3 = 'Ditolak' WHERE kodeFPK = ?";
            ...
            if ($stmt_update_persetujuan->execute()) {
                ...
                $sql_update_fpk = "UPDATE fpk SET PersetujuanDireksi3 = 'Ditolak' WHERE id_fpk = ?";
                $stmt_update_fpk = $connection->prepare($sql_update_fpk);
                $stmt_update_fpk->bind_param("i", $id_fpk);

                if ($stmt_update_fpk->execute()) {
                    // Jika terdapat nilai yang ditolak, ubah status_penyetujuan menjadi "Pending"
                    if ($persetujuan_direksi3 == 'Ditolak') {
            ...
            // Tutup statement pembaruan PersetujuanDireksi3
            $stmt_update_persetujuan->close();
        } else {
            // Jika belum ada, tambahkan baris baru dengan kodeFPK tersebut
            $sql_insert_persetujuan = "INSERT INTO persetujuan (kodeFPK, PersetujuanUser, PersetujuanAtasan, PersetujuanDireksi2, PersetujuanDireksi3, PersetujuanAdmin, Status_Penyetujuan) 
