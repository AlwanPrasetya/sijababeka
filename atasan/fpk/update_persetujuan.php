...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result_select_kodeFPK->num_rows > 0) {
        ...
        if ($result_check_employee->num_rows > 0) {
            ...
            $sql_update_persetujuan = "UPDATE persetujuan SET PersetujuanAtasan = 'Disetujui', Status_Penyetujuan = ? WHERE kodeFPK = ?";
            ...
            if ($stmt_update_persetujuan->execute()) {
                ...
                $sql_update_fpk = "UPDATE fpk SET persetujuanAtasan = 'Disetujui' WHERE id_fpk = ?";
                $stmt_update_fpk = $connection->prepare($sql_update_fpk);
                $stmt_update_fpk->bind_param("i", $id_fpk);

                if ($stmt_update_fpk->execute()) {
                    // Set tanggal atasan jika persetujuan HR Unit disetujui
                    if ($stmt_update_fpk->affected_rows > 0) {
                        $today = date("Y-m-d"); // Ambil tanggal hari ini

                        // Update field tglatasan di tabel fpk
                        $sql_update_tgl_atasan = "UPDATE fpk SET tglAtasan = ? WHERE id_fpk = ?";
                        $stmt_update_tgl_atasan = $connection->prepare($sql_update_tgl_atasan);
                        $stmt_update_tgl_atasan->bind_param("si", $today, $id_fpk);
                        
                        if ($stmt_update_tgl_atasan->execute()) {
                            echo "success";
                        } else {
                            echo "error_update_tgl_atasan";
                        }

                        // Tutup statement pembaruan tglatasan
                        $stmt_update_tgl_atasan->close();
                    }
                } else {
