...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  $id = $_POST['id'];
  $tglCorpHr = date('Y-m-d'); // Mendapatkan tanggal hari ini dalam format YYYY-MM-DD

  // Start transaction
  $connection->begin_transaction();

  try {
    // Update status persetujuan, persetujuanCorpHr, dan tglCorpHr di tabel hcc
    $sql_hcc = "UPDATE hcc SET Status_Penyetujuan='Approved', persetujuanCorpHr='Disetujui', tglCorpHr=? WHERE fpk_selection=?";
    ...
    $stmt_hcc->bind_param("ss", $tglCorpHr, $id);
    ...
    $sql_persetujuan_hc = "UPDATE persetujuan_hc SET persetujuanCorpHr='Disetujui' WHERE fpk_selection=?";
