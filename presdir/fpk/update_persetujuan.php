...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result_select_kodeFPK->num_rows > 0) {
        ...
        if ($result_check_employee->num_rows > 0) {
            ...
            $sql_update_persetujuan = "UPDATE persetujuan SET PersetujuanPresdir = 'Disetujui', Status_Penyetujuan = ? WHERE kodeFPK = ?";
            ...
            if ($stmt_update_persetujuan->execute()) {
                ...
                $sql_update_fpk = "UPDATE fpk SET persetujuanPresdir = 'Disetujui' WHERE id_fpk = ?";
                $stmt_update_fpk = $connection->prepare($sql_update_fpk);
                $stmt_update_fpk->bind_param("i", $id_fpk);

                if ($stmt_update_fpk->execute()) {
                    // Set tanggal Presdir jika persetujuan HR Unit disetujui
                    if ($stmt_update_fpk->affected_rows > 0) {
                        $today = date("Y-m-d"); // Ambil tanggal hari ini

                        // Update field tglPresdir di tabel fpk
                        $sql_update_tgl_presdir = "UPDATE fpk SET tglPresdir = ? WHERE id_fpk = ?";
                        $stmt_update_tgl_presdir = $connection->prepare($sql_update_tgl_presdir);
                        $stmt_update_tgl_presdir->bind_param("si", $today, $id_fpk);

                        if ($stmt_update_tgl_presdir->execute()) {
                            echo "success";
                        } else {
                            echo "error_update_tgl_presdir";
                        }

                        // Tutup statement pembaruan tglpresdir
                        $stmt_update_tgl_presdir->close();
                    }
                } else {
            ...
            $sql_insert_persetujuan = "INSERT INTO persetujuan (kodeFPK, persetujuanUser, PersetujuanAtasan, PersetujuanDireksi2, PersetujuanDireksi3, PersetujuanAdmin, PersetujuanPresdir, PersetujuanCorpHr, PersetujuanSuperadmin, Status_Penyetujuan) 
