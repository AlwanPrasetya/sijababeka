...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  $id = $_POST['id'];
  $tglPresdir = date('Y-m-d'); // Mendapatkan tanggal hari ini dalam format YYYY-MM-DD

  // Start transaction
  $connection->begin_transaction();

  try {
    // Update status persetujuan, persetujuanPresdir, dan tglPresdir di tabel hcc
    $sql_hcc = "UPDATE hcc SET Status_Penyetujuan='Approved', persetujuanPresdir='Disetujui', tglPresdir=? WHERE fpk_selection=?";
    ...
    $stmt_hcc->bind_param("ss", $tglPresdir, $id);
    ...
    $sql_persetujuan_hc = "UPDATE persetujuan_hc SET persetujuanPresdir='Disetujui' WHERE fpk_selection=?";
