...
<body>
          ...
          <table class="table table-bordered bg-white" id="myTable">
            ...
            <tbody>
              ...
              if (isset($_GET['branches'])) {
                ...
                $sql = "SELECT hcc.*, persetujuan_hc.Status_Penyetujuan, persetujuan_hc.persetujuanAtasan, persetujuan_hc.persetujuanPresdir 
            FROM hcc 
  ...
  if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
      ...
      // Menambahkan tombol Reject
      echo "<button type='button' class='btn btn-primary approve-btn' data-id='" . $row["fpk_selection"] . "' data-persetujuanpresdir='" . $row["persetujuanPresdir"] . "'>Approve <svg xmlns='http://www.w3.org/2000/svg' width='1.2em' height='1.2em' viewBox='0 0 24 24'>
        <rect width='24' height='24' fill='none' />
    ...
    $('.approve-btn').click(function() {
      ...
      $.ajax({
        ...
        success: function(response) {
          ...
          var persetujuanPresdir = button.data('persetujuanpresdir');
