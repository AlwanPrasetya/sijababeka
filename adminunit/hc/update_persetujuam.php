...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  $id = $_POST['id'];
  $tglAdmin = date('Y-m-d'); // Mendapatkan tanggal hari ini dalam format YYYY-MM-DD

  // Start transaction
  $connection->begin_transaction();

  try {
    // Update status persetujuan, persetujuanAdmin, dan tglAdmin di tabel hcc
    $sql_hcc = "UPDATE hcc SET Status_Penyetujuan='Approved', persetujuanAdmin='Disetujui', tglAdmin=? WHERE fpk_selection=?";
    ...
    $stmt_hcc->bind_param("ss", $tglAdmin, $id);
