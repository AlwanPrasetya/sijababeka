...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result_select_kodeFPK ->num_rows > 0) {
        ...
        if ($result_check_employee->num_rows > 0) {
            // Jika sudah ada, lakukan pembaruan nilai persetujuanAdmin
            $sql_update_persetujuan = "UPDATE persetujuan SET persetujuanAdmin = 'Ditolak' WHERE kodeFPK = ?";
            ...
            if ($stmt_update_persetujuan->execute()) {
                ...
                $sql_update_fpk = "UPDATE fpk SET persetujuanAdmin = 'Ditolak' WHERE id_fpk = ?";
                $stmt_update_fpk = $connection->prepare($sql_update_fpk);
                $stmt_update_fpk->bind_param("i", $id_fpk);

                if ($stmt_update_fpk->execute()) {
                    // Set status_persetujuan menjadi "Pending" di tabel persetujuan
                    ...
                    // Tutup statement pembaruan status_persetujuan
            ...
            $sql_insert_persetujuan = "INSERT INTO persetujuan (kodeFPK, persetujuanAdmin, PersetujuanAdmin, Status_Penyetujuan) 
