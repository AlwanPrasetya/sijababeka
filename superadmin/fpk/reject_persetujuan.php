...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Lakukan query untuk mendapatkan kodeFPKFPK pegawai dari tabel fpk
    ...
    if ($result_select_kodeFPK->num_rows > 0) {
        ...
        if ($result_check_employee->num_rows > 0) {
            // Jika sudah ada, lakukan pembaruan nilai PersetujuanAtasan dan PersetujuanAdmin
            $sql_update_persetujuan = "UPDATE persetujuan SET PersetujuanAtasan = 'Ditolak', persetujuanAdmin = 'Ditolak' WHERE kodeFPK = ?";
            ...
            if ($stmt_update_persetujuan->execute()) {
                ...
                $sql_update_fpk = "UPDATE fpk SET persetujuanAtasan = 'Ditolak', persetujuanAdmin = 'Ditolak' WHERE id_fpk = ?";
                $stmt_update_fpk = $connection->prepare($sql_update_fpk);
                $stmt_update_fpk->bind_param("i", $id_fpk);

                if ($stmt_update_fpk->execute()) {
                    // Set status_penyetujuan menjadi "Pending" di tabel persetujuan
                    $sql_update_status = "UPDATE persetujuan SET Status_Penyetujuan = 'Pending' WHERE kodeFPK = ?";
            ...
            // Tutup statement pembaruan PersetujuanAtasan dan PersetujuanAdmin
            ...
            $sql_insert_persetujuan = "INSERT INTO persetujuan (kodeFPK, PersetujuanAtasan, persetujuanAdmin, Status_Penyetujuan) 
                                       VALUES (?, 'Ditolak', 'Ditolak', 'Pending')";
