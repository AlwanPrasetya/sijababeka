...
<body>
            ...
            $whereClause";
              ...
              if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                  ...
                  echo "<button type='button' class='btn btn-warning detail-btn center-icon' data-toggle='modal' data-target='#detailModal" . $row["fpk_selection"] . "' style='margin-right: 5px; align-items: center;'>
  ...
  if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
      ...
      // Menambahkan tombol Reject
      echo "<button type='button' class='btn btn-primary approve-btn' data-id='" . $row["fpk_selection"] . "' data-persetujuansuperadmin='" . $row["persetujuanSuperadmin"] . "'>Approve <svg xmlns='http://www.w3.org/2000/svg' width='1.2em' height='1.2em' viewBox='0 0 24 24'>
        <rect width='24' height='24' fill='none' />
