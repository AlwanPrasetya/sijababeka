...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  $id = $_POST['id'];
  $tglUser = date('Y-m-d'); // Mendapatkan tanggal hari ini dalam format YYYY-MM-DD

  // Start transaction
  $connection->begin_transaction();

  try {
    // Update status persetujuan, persetujuanUser, dan tglUser di tabel hcc
    $sql_hcc = "UPDATE hcc SET Status_Penyetujuan='Approved', persetujuanUser='Disetujui', tglUser=? WHERE fpk_selection=?";
    ...
    $stmt_hcc->bind_param("ss", $tglUser, $id);
    ...
    $sql_persetujuan_hc = "UPDATE persetujuan_hc SET persetujuanUser='Disetujui' WHERE fpk_selection=?";
